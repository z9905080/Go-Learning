...
// RandomTestBase 生成若干个不重复的随机数
func RandomTestBase() {
	//测试5次
	//for i := 0; i < 1; i++ {
	nums := generateRandomNumber(100000, 999999, 899998)
...
func generateRandomNumber(start int, end int, count int) []int {
	...
	for len(nums) < count {
		...
		if TimeStamp.Add(time.Second * 1).Before(time.Now()) {
			fmt.Println(len(nums), " : 增加了 ", len(nums)-currentCount, ", ", TimeStamp)
			TimeStamp = time.Now()
			currentCount = len(nums)
		}
		//查重
